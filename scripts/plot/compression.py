"""Compression ratio vs vocabulary size for each tokeniser type.

Loads results from the compression CSV and generates a publication-quality
PDF with one subplot per tokeniser type, showing compression ratio across
...
def _finalize_plot_layout(fig: Figure, axes: list, n_tokenisers: int) -> None:
    ...
    fig.supylabel("Compression ratio", fontsize=8)
