"""OOV rate vs vocabulary size for each tokeniser type.

Loads results from the OOV CSV and generates a publication-quality PDF
with one subplot per tokeniser type, showing out-of-vocabulary rate across
...
FIGURE_PATH = FIGURES_DIR / "oov.pdf"
...
def create_oov_plot(df: pd.DataFrame, figsize: tuple[float, float] = (5.5, 1.8)) -> Figure:
    ...
    for i, tokeniser_type in enumerate(tokeniser_types):
...
def plot(
    ...
    figsize: tuple[float, float] = (5.5, 1.8),
    show: bool = False,
) -> None:
    df = load_data(input_path)
    fig = create_oov_plot(df, figsize)
